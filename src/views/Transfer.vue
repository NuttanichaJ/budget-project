      ...
      <b-nav class="mt-3">
        <b-navbar-nav class="mt-2 mb-2 mr-sm-2 mb-sm-0 mr-auto">
          <b-nav-form>
            <b-input-group>
              <b-button
                class="mb-2 ml-sm-2 mb-sm-0 mr-1"
                variant="primary"
                id="history-undo"
                v-b-tooltip.hover
                title="เลิกทำ"
      ...
      <b-nav class="mt-3">
        <b-navbar-nav class="mb-2 mr-sm-2 mb-sm-0 mr-auto">
          <b-form inline class="nav-text">
            <label
              class="mb-2 ml-sm-2 mb-sm-0 mr-4"
              for="form-custom-select-pref"
              >โอนเงินเข้า</label
            >
            <b-form-select
              class="font-18 rounded-2 mb-2 mr-sm-5 mb-sm-0"
              id="select-ProjectIn"
              @change="retrieveTransfer()"
              v-model="selectedProjectIn"
...
export default {
  ...
  mounted() {
    ...
    // var getIncomeAndOutcomeMainProject = function(MP_ID) {
    //     // SubprojectDataService.get(SP_ID_Out).then((response) => {
    ...
    //     MainprojectDataService.get(MP_ID).then((response) => {
    //       var Subprojects = response.data.subprojects;
    //       for (var i in Subprojects) {
    //         var SP_ID = Subprojects[i].SP_ID
    //         var SP_Income = Subprojects[i].SP_Income;
    //         this.MP_Income.push({SP_ID: SP_ID, SP_Income: SP_Income}) ;
    //         var SP_Outcome = Subprojects[i].SP_Outcome;
    //         this.MP_Outcome.push({SP_ID: SP_ID, SP_Outcome: SP_Outcome})
    //       }
    //     });
    // };
    ...
    selectEl.addEventListener("change", function () {
      ...
      //store SP_Income for Calc MP_Outcome, MP_Income
      var selectedMP_ID = selectedProjectIn[0];
      MP_Income = []
      MP_Outcome = []
      MainprojectDataService.get(selectedMP_ID).then((response) => {
          var Subprojects = response.data.subprojects;
          for (var i in Subprojects) {
            var SP_ID = Subprojects[i].SP_ID
            var SP_Income = Subprojects[i].SP_Income;
            MP_Income.push({SP_ID: SP_ID, SP_Income: SP_Income}) ;
            var SP_Outcome = Subprojects[i].SP_Outcome;
            MP_Outcome.push({SP_ID: SP_ID, SP_Outcome: SP_Outcome});
          }
      });
              ...
              if (selectedProjectIn[1] == "SP_ID") {
                ...
                if (selectedProjectIn[0] == response.data[i].MP_ID) {
                  ...
                  for (j in response.data[i].subprojects) {
                    if (
                      selectedProjectIn[2] !=
                      response.data[i].subprojects[j].SP_ID
                    ) {
                      //console.log(response.data[i].subprojects[j].SP_ID)
                      optionsProjectOut[index] =
                        "S" +
                        response.data[i].subprojects[j].SP_ID +
                        ": " +
                        response.data[i].subprojects[j].SP_Name;
                      index++;
                    }
                  }
                }
      ...
      TransferDataService.getAll().then((response) => {
        var i;
        //var index = 0;
        for (i in response.data) {
          //if(dapartment_id == response.data[i].D_ID){
            ...
            if (selectedProjectIn[2] == response.data[i].SP_ID_In) {
              ...
              if (response.data[i].SPtranfers_Out != null) {
                ...
                for (index4 in optionsProjectOut) {
                  ...
                  //console.log(getoptionsProjectOut)
                  if (
                    getoptionsProjectOut[1] ==
                    response.data[i].SPtranfers_Out.SP_Name
                  ) {
      ...
      //Set Data Filter for Table(Tabulator)
      if (selectedProjectIn[0] == "MP_ID") {
    ...
    var printDelIcon = function (cell, formatterParams, onRendered) {
      //plain text value
    ...
    table = new Tabulator("#table", {
      ...
      rowAdded: function (row) {
        ...
        if (selectedProjectIn[0] == "MP_ID") {
          ...
          data.SP_ID_In = SP_ID;
          data.MP_ID_In = selectedProjectIn[0];
        }
        listEdit.push(data);
      },
        ...
        },
        {
          ...
          cellClick: function (e, cell) {
            if (
              confirm(
                "ต้องการลบ " +
                  cell.getRow().getData().ProjectOutcome +
                  " ใช่หรือไม่?"
              ) == true
            ) {
              cell.getRow().delete();
  ...
  methods: {
    save() {
      this.$confirm("คุณต้องการบันทึกใช่หรือไม่?", " ", "question").then(() => {
        var total_Amount = parseFloat(table.getCalcResults().top.Amount); //Add Amount => For Income
        // check project outcome id
        var project = [];
        for (var i in listEdit) {
          listEdit[i].SP_Income = total_Amount;
          const action = listEdit[i].Action;
          if (action == "add" || action == "edit") {
            if (
              listEdit[i].ProjectOutcome != undefined &&
              listEdit[i].ProjectOutcome.includes(":")
            ) {
              ...
              if (project[0].includes("M")) {
                ...
                listEdit[i].MP_ID_Out = listEdit[i].MP_ID_In;
              }
            }
          }
        }
        ...
        for (i in listEdit) {
          ...
          if (action == "edit") {
            ...
            this.updateTransferToMainAndSubproject2(listEdit[i]);
          } else if (action == "del") {
            this.deleteTransfer(edit_ID);
            this.updateTransferToMainAndSubproject3(listEdit[i]);
          } else if (action == "add") {
            //Record add history
    ...
    cancel() {
      this.$confirm("คุณต้องการยกเลิกใช่หรือไม่?", " ", "error").then(() => {
        listEdit = [];
        window.location.reload();
        //do something...
      });
        ...
        .then((response) => {
          ...
          for (i in response.data) {
            if (this.user.depart_id == response.data[i].D_ID) {
              ...
              // Subproject
              for (j in response.data[i].subprojects) {
                this.optionsProjectChild.push({
                  value:
                    response.data[i].MP_ID +
                    ":SP_ID:" +
                    response.data[i].subprojects[j].SP_ID,
                  text: response.data[i].subprojects[j].SP_Name,
                });
              }
              this.optionsProject.push({
                label: response.data[i].MP_Name,
                options: this.optionsProjectChild,
              });
            }
          }
        })
        ...
        .then((response) => {
          ...
          for (i in response.data) {
            if (response.data[i].MPtranfers_Out != undefined) {
              this.tableData[i].ProjectOutcome =
              response.data[i].MPtranfers_Out.MP_Name;
              // this.tableData[i].ProjectIncome = response.data[i].MPtranfers_in.MP_Name;
            } if (response.data[i].SPtranfers_Out != undefined) {
              this.tableData[i].ProjectOutcome =
              response.data[i].SPtranfers_Out.SP_Name;
        ...
        for (var k in listEdit) {
          if (k != j) {
            ...
            if (MP_ID_In1 == MP_ID_In2 && SP_ID_In2 == SP_ID_In1) {
              if (
                [MP_ID_In1, MP_ID_In2].every((x) => x !== null) ||
                [SP_ID_In2, SP_ID_In1].every((x) => x !== null)
              ) {
                if (MP_ID_Out1 == MP_ID_Out2 && SP_ID_Out1 == SP_ID_Out2) {
                  if (
                    [MP_ID_Out1, MP_ID_Out2].every((x) => x !== null) ||
                    [SP_ID_Out1, SP_ID_Out2].every((x) => x !== null)
                  ) {
    ...
    updateTransferToMainAndSubproject3(data) {
      var SP_Income = data.SP_Income;
      var SP_ID_In1 = data.SP_ID_In;
      var SP_ID_Out1 = data.SP_ID_Out;
      //var MP_ID_In1 = data.MP_ID_In;
      var MP_ID_Out1 = data.MP_ID_Out;

      var SP_Outcome = 0;
      for(var i in this.tableData){
        //var action = this.tableData[i].Action
        var SP_ID_Out2 = this.tableData[i].SP_ID_Out
        var action = this.tableData[i].Action
          if(SP_ID_Out1 == SP_ID_Out2 && action != 'del'){
            SP_Outcome += this.tableData[i].Amount
            //console.log(this.tableData[i].Amount)
          }
      }
      //Main Project
      var totalMP_Income = 0;
      //var totalMP_Outcome = 0;
      for(var j in MP_Income){
        if(SP_ID_In1 == MP_Income[j].SP_ID){
          MP_Income[j].SP_Income = SP_Income
        }
        totalMP_Income += MP_Income[j].SP_Income
      }

      // for(var k in MP_Outcome){
      //   if(SP_ID_Out1 == MP_Outcome[k].SP_ID){
      //     MP_Outcome[k].SP_Outcome = SP_Outcome
      //   }
      //   totalMP_Outcome += MP_Outcome[k].SP_Outcome
      // }

      SubprojectDataService.update(SP_ID_Out1, { SP_Outcome: SP_Outcome });
      SubprojectDataService.update(SP_ID_In1, { SP_Income: SP_Income });
      MainprojectDataService.update(MP_ID_Out1, { MP_Income: totalMP_Income, MP_Outcome: totalMP_Income })
      // MainprojectDataService.update(MP_ID_In1, { MP_Income: totalMP_Income })
    },

    updateTransferToMainAndSubproject2(data){
      var SP_Income = data.SP_Income;
      var SP_ID_In1 = data.SP_ID_In;
      var SP_ID_Out1 = data.SP_ID_Out;
      // var MP_ID_In1 = data.MP_ID_In;
    ...
    updateTransferToMainAndSubproject(data) {
      ...
      var tableSP_Outcome = 0;
      for(var i in this.tableData){
        //var action = this.tableData[i].Action
        var SP_ID_Out2 = this.tableData[i].SP_ID_Out
          if(SP_ID_Out1 == SP_ID_Out2){
            tableSP_Outcome += this.tableData[i].Amount
          }
      }
      var SP_Outcome = tableSP_Outcome + data.Amount
    ...
    //     // Update Subproject
    //     // Update SP_Income
    //     SubprojectDataService.update(SP_ID_In, { SP_Income: SP_Income });
    //     // // Update Subproject
    //     // // Update SP_Outcome
    //     // SubprojectDataService.get(SP_ID_Out).then((response) => {
    //     //   var transfers_Out = response.data.transfers_Out;
    ...
    //     // var MP_ID_In = data.MP_ID_In;

    //     // //Update Mainproject
    //     // //Update MP_Income
    //     // //Update MP_Outcome
    //     // MainprojectDataService.get(MP_ID_In).then((response) => {
    //     //   var Subprojects = response.data.subprojects;
    //     //   var totalIncome = 0;
    //     //   var totalOutcome = 0;
    //     //   for (var i in Subprojects) {
    ...
    //     //   }
    //     //   MainprojectDataService.update(MP_ID_In, {
    //     //     MP_Income: totalIncome,
    //     //     MP_Outcome: totalOutcome,
    //     //   });
    //     // });
    //   }
