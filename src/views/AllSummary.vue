<template>
  <div >
    <SummaryHead/>
    <div class="allsummary">    
        <b-container fluid>
          <!-- สาขาวิชา -->
          <a class="brancebtn btn btn-block" href="/SubSummary">
          <div class="mt-2 mx-3" id="branch">
            <b-row class="mt-2 d-flex align-items-center">              
              <b-col cols="4" class="border border-dark rounded-left p-2">
                  <div class="d-flex flex-wrap justify-content-around">  
                    <div class="d-flex align-items-center">
                      <h4 class="border border-dark rounded p-2">CE</h4>
                    </div>
                    <div class="">
                      <p class="mb-0"><small>โครงการย่อยที่กำลังดำเนินการ</small></p>
                      <h2 class="mb-0 text-center">5</h2>
                      <p class="text-muted mt-0 mb-0 float-right"><small>โครงการ</small></p>
                    </div>
                    <div class="">
                      <p class="mb-0"><small>โครงการย่อยที่เสร็จสิ้น</small></p>
                      <h2 class="mb-0 text-center">3</h2>
                      <p class="text-muted mt-0 mb-0 float-right"><small>โครงการ</small></p>
                    </div>
                    
                  </div>  
              </b-col> 
                        
              <b-col cols="8" class="border border-dark rounded-right p-2 ">
                <div class="d-flex flex-wrap justify-content-around">                  
                   <div class="">
                    <p class="mb-0"><small>งบประมาณทั้งหมด (ตามแผน)</small></p>
                    <h2 class="mb-0 text-center">2,881,340</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0"><small>เบิกจ่ายทั้งหมด</small></p>
                    <h2 class="mb-0 text-center">0</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0 "><small>งบประมาณคงเหลือ (ตามแผน)</small></p>
                    <h2 class="mb-0 text-center">2,881,340</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0"><small>งบประมาณคงเหลือ (เบิกจ่ายจริง)</small></p>
                    <h2 class="mb-0 text-center">2,881,340</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                </div> 
              </b-col>
            </b-row>
          </div>
          </a>

          <a class="brancebtn btn btn-block" href="/SubSummary">
            <div class="mt-2 mx-3" id="branch">
            <b-row class="mt-3 d-flex align-items-center">              
              <b-col cols="4" class="border border-dark rounded-left p-2">
                  <div class="d-flex flex-wrap justify-content-around">  
                    <div class="d-flex align-items-center">
                      <h4 class="border border-dark rounded p-2">EE</h4>
                    </div>
                    <div class="">
                      <p class="mb-0"><small>โครงการย่อยที่กำลังดำเนินการ</small></p>
                      <h2 class="mb-0 text-center">11</h2>
                      <p class="text-muted mt-0 mb-0 float-right"><small>โครงการ</small></p>
                    </div>
                    <div class="">
                      <p class="mb-0"><small>โครงการย่อยที่เสร็จสิ้น</small></p>
                      <h2 class="mb-0 text-center">20</h2>
                      <p class="text-muted mt-0 mb-0 float-right"><small>โครงการ</small></p>
                    </div>
                    
                  </div>  
              </b-col> 
                        
              <b-col cols="8" class="border border-dark rounded-right p-2 ">
                <div class="d-flex flex-wrap justify-content-around">                  
                   <div class="">
                    <p class="mb-0"><small>งบประมาณทั้งหมด (ตามแผน)</small></p>
                    <h2 class="mb-0 text-center">1,667,400</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0"><small>เบิกจ่ายทั้งหมด</small></p>
                    <h2 class="mb-0 text-center">27,750</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0 "><small>งบประมาณคงเหลือ (ตามแผน)</small></p>
                    <h2 class="mb-0 text-center">1,667,400</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0"><small>งบประมาณคงเหลือ (เบิกจ่ายจริง)</small></p>
                    <h2 class="mb-0 text-center">1,639,650</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                </div> 
              </b-col>
            </b-row>
          </div>
          </a>
          
          <a class="brancebtn btn btn-block" href="/SubSummary">
          <div class="mt-2 mx-3" id="branch">
            <b-row class="mt-3 d-flex align-items-center">              
              <b-col cols="4" class="border border-dark rounded-left p-2">
                  <div class="d-flex flex-wrap justify-content-around">  
                    <div class="d-flex align-items-center">
                      <h4 class="border border-dark rounded p-2">AE</h4>
                    </div>
                    <div class="">
                      <p class="mb-0"><small>โครงการย่อยที่กำลังดำเนินการ</small></p>
                      <h2 class="mb-0 text-center">5</h2>
                      <p class="text-muted mt-0 mb-0 float-right"><small>โครงการ</small></p>
                    </div>
                    <div class="">
                      <p class="mb-0"><small>โครงการย่อยที่เสร็จสิ้น</small></p>
                      <h2 class="mb-0 text-center">9</h2>
                      <p class="text-muted mt-0 mb-0 float-right"><small>โครงการ</small></p>
                    </div>
                    
                  </div>  
              </b-col> 
                        
              <b-col cols="8" class="border border-dark rounded-right p-2 ">
                <div class="d-flex flex-wrap justify-content-around">                  
                   <div class="">
                    <p class="mb-0"><small>งบประมาณทั้งหมด (ตามแผน)</small></p>
                    <h2 class="mb-0 text-center">889,280</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0"><small>เบิกจ่ายทั้งหมด</small></p>
                    <h2 class="mb-0 text-center">50,755</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0 "><small>งบประมาณคงเหลือ (ตามแผน)</small></p>
                    <h2 class="mb-0 text-center">989,280</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                  <div class="">
                    <p class="mb-0"><small>งบประมาณคงเหลือ (เบิกจ่ายจริง)</small></p>
                    <h2 class="mb-0 text-center">938,525</h2>
                    <p class="text-muted mt-0 mb-0 float-right"><small>บาท</small></p>
                  </div>
                </div> 
              </b-col>
            </b-row>
          </div>
          </a>

        </b-container>     
      </div>

  </div>
</template>

<script>
import SummaryHead from "@/components/SummaryHead.vue"

export default {
  name: "AllSummary",
    components :{
        SummaryHead
    },
    data() {
      return {
        selectedYear: "2564",
        optionsYear: [
          { value: '2564', text: '2564' },
          { value: '2563', text: '2563' },
          { value: '2562', text: '2562' },
          { value: '2561', text: '2561' }
        ]
      }
    },

}

</script>

<style scoped>
.brancebtn:focus {
    outline: none !important;
    border: none !important;
    -webkit-box-shadow: none !important;
    box-shadow: none !important;
}

.summary{
  margin: 20px;
  margin-left: 30px;
  margin-right: 30px;
  
}
.baht{
  float: right;
  margin-bottom: 0px;
  margin-top: 0px;

}
#border{
  display: block;
  padding: 10px;  
  border: 0.3px solid black;
  border-radius: 5px;
  width: 210px;

}


</style>