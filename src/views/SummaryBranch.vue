<template>
    <div id="subsummarybranch">
        <div>
            <h4 class="mb-3 " v-if="this.user.permission == 'สาขาวิชา' || this.user.permission == 'ส่วนกลาง' ">{{ user.depart_name}}</h4>
...
export default {
    ...
    // table
    mounted(){
        this.getAllProjects(this.user.depart_id);
                ...
                MainprojectDataservice.getAll()
                    .then(response => {
                        ...
                        for(var i in response.data) {
                            if(response.data[i].D_ID == UserD_ID){
